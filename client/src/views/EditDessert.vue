  ...
  <v-container>
    <v-row no-gutters>
      <v-col cols="10" class="mx-auto">
        <v-card class="pa-5">
          <v-card-title>แก้ไขขนมไทย</v-card-title>
          <v-divider></v-divider>
          <v-form ref="form" @submit.prevent="updateForm" class="pa-5" enctype="multipart/form-data">
            ...
            <v-file-input @change="selectFile" show-size-counter multiple label="เลือกรูปภาพ"></v-file-input>
            <v-img :src="`/${dessert.image}`" width="120"></v-img>

            <v-btn type="submit" class="mt-3" color="success">แก้ไขขนม</v-btn>
          </v-form>
        </v-card>
      </v-col>
    </v-row>
